...
# run tests
CYPRESS_baseUrl=$CYPRESS_baseUrl NO_COLOR=1 cypress run --reporter json-stream --browser chrome
...
if [[ "$?" -ne 0 ]]; then
    ...
    echo ERROR: Cypress E2E tests have failed. Screenshots will be uploaded to $COPY_DESTINATIONscreenshots 
