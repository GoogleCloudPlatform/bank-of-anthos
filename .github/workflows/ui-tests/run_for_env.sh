...
if [[ -z "${CYPRESS_baseUrl}" ]]; then
    ...
    if [[ "$ANTHOS_MEMBERSHIP_SHORT" == "staging-membership" ]]; then
        ...
        export CYPRESS_baseUrl=http://$(kubectl get ingress frontend-ingress --namespace bank-of-anthos-production -o jsonpath='{.status.loadBalancer.ingress[0].ip}') # TODO: consider switching to domain name
    else
