...
if [ ! "`kubectl get ns | grep istio-system; echo $?`" ]; then
  echo "Istio does not appear to be installed.  Download the istioctl client"
  echo " and install for your environemnt"
  echo "See: https://istio.io/latest/docs/setup/getting-started/#download"
  exit 1
else
  echo "istio appears to be installed"
fi
...
if [ ! "`kubectl get ns ${NAMESPACE:-bank-of-anthos-opsani} >& /dev/null; echo $?`" ]; then
  ...
  echo ""
  echo "Adding the istio label to the namespace"
  ...
  echo "adding istio-injection to namespace $NAMESPACE"
  kubectl label namespace $NAMESPACE istio-injection=enabled --overwrite
fi
