...
if [ -z "$(kubectl get deploy servo)" ]; then
...
kubectl exec -c servo deploy/servo -- servo --token-file /servo/opsani.token inject-sidecar -n ${NAMESPACE} -s ${SERVICE} deployment/${DEPLOYMENT}
./update-svc.sh envoy